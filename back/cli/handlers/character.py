...
class CharacterHandler:
    ...
    def show_inventory(self):
        self.nav.logs.clear()
        self.nav.log(f'{self.character.name} inventory : {self.character.inventory.total}/{self.character.inventory.max_items}')
        for item in self.character.inventory.items.values():
            self.nav.log(f'{item.quantity}x {item.name} ({item.type}|{item.subtype}) C{list(item.crafts.keys())!s} E{list(item.effects.keys())!s}')
    ...
    async def charcter_menu(self):
        while True:
            ...
            if self.nav.pool.is_running(self.character.name):
                options.append('Stop routine')
            else:
                options.append('Assign role',)
            ...
            if choice.lower() in ('b','back'):
                ...
                if choice == '1':
                    self.show_inventory()
                elif self.nav.pool.is_running(self.character.name) and choice == '2':
                    await self.nav.pool.stop_and_wait(self.character.name)
                else:
                    if choice == '2':
                        await self.assign_role()
                    elif self.character.role.className != 'Peasant' and choice == '3':
